...
export function ImpactPanel({ impact }: ImpactPanelProps) {
    ...
    return (
      <div className="p-4 space-y-3">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 text-center">
          <p className="text-sm text-slate-400 italic">{t('impact.noImpact')}</p>
        </div>
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      >
        <span className="text-xs text-slate-400">{t('impact.estimatedImpact')}</span>
      ...
      {/* Explanation */}
      {explanation && <p className="text-xs text-slate-400 leading-relaxed">{explanation}</p>}
      ...
      <div>
        ...
        <ul className="space-y-1 max-h-40 overflow-y-auto">
          {affectedNodes.map((nodeId) => (
            <li
              key={nodeId}
              className="text-xs text-slate-300 font-mono flex items-center gap-1.5 bg-slate-800 rounded px-2 py-1"
            >
