...
```json
{
  ...
  "dependencies": [{ "node_id": "uuid", "label": "UserRepository.ts", "type": "repository" }],
  "dependents": [{ "node_id": "uuid", "label": "AuthRoutes.ts", "type": "route" }],
  "node_type": "service"
}
```
...
```json
{
  "nodes": [{ "id": "uuid", "label": "AuthService.ts", "path": "...", "type": "service" }],
  "total_hits": 5
}
```
...
### 4.1 Extensiones implementadas en v2 (2026-06-01)

- `get_architecture_detection(projectId) → ArchitectureDetectionResult` — implementado PR2
- `get_impact_analysis(projectId, nodeId) → ImpactAnalysisResult` — implementado PR3
- `get_graph_insights(projectId) → GraphInsights` — implementado PR3
- `get_export(projectId, format) → ExportPayload` — implementado PR4

> Verificado en verify-report.md (PASS) y CHANGELOG_CONTRATOS.md (v2.0 implementado).
...
_Documento pre-SDD. Versión inicial. Se refinará durante fase `spec` del SDD v1._
